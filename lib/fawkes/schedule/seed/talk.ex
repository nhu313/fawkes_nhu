defmodule Fawkes.Schedule.Seed.Talk do
  ...
  def perform do
    ...
    |> Map.put(:categories, fetch_model_by_slug(Category))
  ...
  # def add_categories(changeset) do
  #   categories = Ecto.Changeset.get_field(changeset, :categories)
  #
  ...
  def data(relations) do
    [
      %{
        ...
        description: "Creator keynote"
      },
      %{
        slug: "slot_5_lance_halvorsen",
        title: "Elixir at a Walking Pace",
        slot_id: relations[:slots]["slot_5"],
        categories: [relations[:categories]["phoenix"], relations[:categories]["production"]],
...
<p>You'll get the most out of this presentation if you have used example-based testing in any language. You'll go from \"property-based testing looks awesome\" to \"I can do it!\"</p>\n"
      ...
      %{
        ...
        slot_id: relations[:slots]["slot_k"],
        categories: [relations[:categories]["code_quality"], relations[:categories]["phoenix"]],
        audience_id: relations[:audiences]["beginner"],
        location_id: relations[:locations]["evergreen_ab"],
...
<p>We hope that after hearing this talk you will be able to identify scenarios where databases won't be needed and thus write a stateless application.</p>\n"
  ...
  defp fetch_all_by_slug(query) do
    ...
    |> Enum.reduce(%{}, fn(model, acc) ->
         Map.put(acc, model.slug, model.id)
       end)
  end

  defp fetch_model_by_slug(query) do
    query
    |> Repo.all
    |> Enum.reduce(%{}, fn(model, acc) ->
         Map.put(acc, model.slug, model)
       end)
  end
end
